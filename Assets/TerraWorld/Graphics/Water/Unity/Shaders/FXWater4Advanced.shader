Shader "TerraUnity/Water4"
{
	Properties
	{
		...
		_FlowMapTiling("Flow Map Tiling", Float) = 100.0
		_FlowMapStrength("Flow Map Strength", Float) = 20.0
		...
		_Shininess ("Shininess", Range (2.0, 5000.0)) = 200.0	
	...
	half4 frag( v2f i ) : SV_Target
	{
		...
		// +x = right  -x = left
		// +y = up     -y = down
		float2 flowmap = (tex2D(_FlowMap, (VERTEX_WORLD_POSITION / _FlowMapTiling).xz).rg - 0.5) * _FlowMapStrength;
