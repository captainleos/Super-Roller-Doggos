...
float3 worldNormal = mul(unity_ObjectToWorld, float4(v.normal, 0.0)).xyz;
	...
	{
		float noise = SimplexNoise(_NoiseTiling5 * float3(v.texcoord.xy, 0));
		uv0 = TilingRemoverVert(_TilingRemover5, uv0, noise);
	}

	if (_TilingRemover6 > 0.0f)
	{
		float noise = SimplexNoise(_NoiseTiling6 * float3(v.texcoord.xy, 0));
		uv1 = TilingRemoverVert(_TilingRemover6, uv1, noise);
	}

	if (_TilingRemover7 > 0.0f)
	{
		float noise = SimplexNoise(_NoiseTiling7 * float3(v.texcoord.xy, 0));
		uv2 = TilingRemoverVert(_TilingRemover7, uv2, noise);
	}

	if (_TilingRemover8 > 0.0f)
	{
		float noise = SimplexNoise(_NoiseTiling8 * float3(v.texcoord.xy, 0));
...
//#endif
	...
	if (_TilingRemover5 > 0.0f)
	{
		float noise = 1.0 - SimplexNoise(float3(_NoiseTiling5, _NoiseTiling5, _NoiseTiling5) * normalize(IN.worldPos));
		uv0 = TilingRemoverSurf(_TilingRemover5, uv0, noise);
	}

	if (_TilingRemover6 > 0.0f)
	{
		float noise = 1.0 - SimplexNoise(float3(_NoiseTiling6, _NoiseTiling6, _NoiseTiling6) * normalize(IN.worldPos));
		uv1 = TilingRemoverSurf(_TilingRemover6, uv1, noise);
	}

	if (_TilingRemover7 > 0.0f)
	{
		float noise = 1.0 - SimplexNoise(float3(_NoiseTiling7, _NoiseTiling7, _NoiseTiling7) * normalize(IN.worldPos));
		uv2 = TilingRemoverSurf(_TilingRemover7, uv2, noise);
	}

	if (_TilingRemover8 > 0.0f)
	{
		float noise = 1.0 - SimplexNoise(float3(_NoiseTiling8, _NoiseTiling8, _NoiseTiling8) * normalize(IN.worldPos));
