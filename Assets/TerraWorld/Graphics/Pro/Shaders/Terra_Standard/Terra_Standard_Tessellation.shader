...
Shader "TerraUnity/Standard Tessellation"
{
	Properties
	{
	    _Tess ("Tessellation", Range(1, 150)) = 1
        _maxDist ("Tess Fade Distance", Range(0, 500.0)) = 0.0
        _ShadowLOD ("Shadow Gen LOD", Range(0, 1.0)) = 0
        _Displacement ("Displacement", Range(0, 128.0)) = 0.0
        _DispOffset ("Disp Offset", Range(0, 1)) = 0.5
        _Phong  ("Phong Smoothing Factor", Range(0, 4)) = 0
        [Enum(Distance Based, 0, Edge Length, 1)] _TessMode ("Tessellation Mode", Float) = 0
	...
	SubShader
	{
		...
		Pass
		{
    		...
    		#include "TerraStandard_Core_Forward_Tessellation.cginc"
		...
		Pass
		{
			...
			#include "TerraStandard_Shadow_Tessellation.cginc"
		...
		Pass
		{
			...
			#include "TerraStandard_UnityStandardMetaMod.cginc"
			ENDCG
		}
	}
	...
	FallBack "VertexLit"
	CustomEditor "TerraStandardTessellation_GUI"
}
