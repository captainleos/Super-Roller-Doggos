...
namespace TerraUnity.Runtime
{
	...
	public class ConvexHullCalculator {
        ...
        public void GenerateHull (List<System.Numerics.Vector3> points, ref List<System.Numerics.Vector3> verts)
        {
            var Unityverts = new List<Vector3>();
            var Unitytris = new List<int>();
            ...
            for (int i = 0; i < points.Count; i++)
                Unitypoints.Add(TUtils.CastToUnity(points[i]));

            GenerateHull(Unitypoints, true, ref Unityverts, ref Unitytris, ref Unitynormals);

            mesh.Clear();
            mesh.SetVertices(Unityverts);
            mesh.SetTriangles(Unitytris, 0);
            mesh.SetNormals(Unitynormals);

            for (int i = 0; i < Unityverts.Count; i++)
                verts.Add(TUtils.CastToNumerics(Unityverts[i]));
        }
		/// <summary>
